 ...
 * PackageName com.werif.specialeffect.custom_view
 */
...
public class RulerView2 extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < ((endNum - startNum) / unitNum) + 1; i++) {
            ...
            paint.setColor(getColor(startColor, endColor, (float) i / (float) ((endNum - startNum) / unitNum)));
            canvas.drawRoundRect(rectF, lineWidth / 2, lineWidth / 2, paint);

            //绘制刻度文字
            if (i % 10 == 0) {
                textPaint.setColor(getColor(startColor, endColor, (float) i / (float) ((endNum - startNum) / unitNum)));
        ...
        indicatorColor = getColor(startColor, endColor, Math.abs((float) (offsetStart + movedX) / (float) (lineSpacing * ((endNum - startNum) / unitNum))));
